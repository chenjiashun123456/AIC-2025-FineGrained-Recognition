...
def main():
    ...
    train_dir = r'd:\Aic\最终的代码\train_400'
    test_dir = None  # 本地检验训练无需测试集

    model_name = 'swin_base'
    # 动态选择训练轮数：大数据集更长；小数据集保持现有配置
    num_epochs = 120
    # 输出隔离：按数据集在本地 checkpoints 根目录下分子目录
    checkpoint_dir_base = os.getenv("CHECKPOINT_DIR", r"d:\Aic\最终的代码\checkpoints")
    dataset_tag = 'train_400'
    ...
    def choose_batch_size(dev, model_tag):
        ...
        # 新增：Swin Base 224 输入的批量推荐（更省显存，可更大一些）
        if model_tag.startswith('swin'):
            if free_gb >= 20: return 64
    ...
    # 预期效果：
    # - 更稳定的收敛；更好的泛化性能
    base_lr = 5e-5  # 从 5e-4 改为 5e-5
    learning_rate = base_lr * (batch_size / 32)
    weight_decay = 0.05  # 从 0.10 改为 0.05
    ...
    # 梯度累积：大数据集目标有效批量更大
    accum_steps = max(1, 128 // batch_size)
